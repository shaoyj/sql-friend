 ...
 * @time: 6:21 PM
 */
public class InStrategy implements SqlConvertStrategy {

    @Override
    public boolean filter(String conditionFlag) {
        return StringUtils.contains(ConditionConstant.IN, conditionFlag);
    }
    ...
    public void wrapperSql(String columnName, StringBuilder sql) {
        sql.append(" AND ").append(columnName)
                .append(" in (:")
                .append(columnName).append(ConditionConstant.IN)
                .append(")");
    }
}
