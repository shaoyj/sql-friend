 ...
 * @time: 6:21 PM
 */
public class LeStrategy implements SqlConvertStrategy {

    @Override
    public boolean filter(String conditionFlag) {
        return StringUtils.contains(ConditionConstant.LE, conditionFlag);
    }
    ...
    public void wrapperSql(String columnName, StringBuilder sql) {
        sql.append(" AND ").append(columnName)
                .append(" <=:")
                .append(columnName).append(ConditionConstant.LE);
    }
}
