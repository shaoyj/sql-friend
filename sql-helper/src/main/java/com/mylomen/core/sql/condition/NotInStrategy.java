 ...
 * @time: 6:21 PM
 */
public class NotInStrategy implements SqlConvertStrategy {

    @Override
    public boolean filter(String conditionFlag) {
        return StringUtils.contains(ConditionConstant.NOT_IN, conditionFlag);
    }
    ...
    public void wrapperSql(String columnName, StringBuilder sql) {
        sql.append(" AND ").append(columnName)
                .append(" not in (:")
                .append(columnName).append(ConditionConstant.NOT_IN)
                .append(") ");
    }
}
